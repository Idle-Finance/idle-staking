...
class HardwareSigner extends ethers.Signer {
  ...
  async signMessage(message) {
    ...
    const sig = await this.retry(`signMessage ${this.i++}`, async () => this.eth.signPersonalMessage(this.path, messageHex));
    sig.r = '0x' + sig.r;
    sig.s = '0x' + sig.s;
  ...
  async signTransaction(transaction) {
    ...
    const sig = await this.retry(`signTransaction ${this.i++}`, async () => this.eth.signTransaction(this.path, unsignedTx));
